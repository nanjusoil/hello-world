...
for(var i = 0 ; i < 7 ; i ++){
  request('https://ifoodie.tw/api/blog/?offset=' + i * 300 + '&limit=300&order_by=-date&city_name=%E5%8F%B0%E5%8C%97', function (error, response, body) {
    ...
    body.response.forEach(function(val){
      ...
      connection.query("INSERT INTO `store` ( `name`, `lat`, `lng`, `address`) VALUES ('" + val.restaurant[0].name  +"', '" + val.lat +"', '" + val.lng  +"', " + val.restaurant[0].address  +");", function (error, results, fields) {
